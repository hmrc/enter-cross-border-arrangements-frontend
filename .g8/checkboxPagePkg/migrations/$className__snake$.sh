...
echo "GET        /$className;format="decap"$                        controllers.$package$.$className$Controller.onPageLoad(mode: Mode = NormalMode)" >> ../conf/app.routes
echo "POST       /$className;format="decap"$                        controllers.$package$.$className$Controller.onSubmit(mode: Mode = NormalMode)" >> ../conf/app.routes

echo "GET        /change$className$                  controllers.$package$.$className$Controller.onPageLoad(mode: Mode = CheckMode)" >> ../conf/app.routes
echo "POST       /change$className$                  controllers.$package$.$className$Controller.onSubmit(mode: Mode = CheckMode)" >> ../conf/app.routes
...
awk '/trait UserAnswersEntryGenerators/ {\
    ...
    print "    Arbitrary {";\
    print "      for {";\
    print "        page  <- arbitrary[pages.$package$.$className$Page.type]";\
    print "        value <- arbitrary[models.$package$.$className$].map(Json.toJson(_))";\
...
awk '/trait PageGenerators/ {\
    print;\
    print "";\
    print "  implicit lazy val arbitrary$className$Page: Arbitrary[pages.$package$.$className$Page.type] =";\
    print "    Arbitrary(pages.$package$.$className$Page)";\
...
awk '/val generators/ {\
    print;\
    print "    arbitrary[(pages.$package$.$className$Page.type, JsValue)] ::";\
...
awk '/extends IndividualRows/ {\
     print;\
     print "";\
     print "  def $className;format="decap"$: Option[Row] = userAnswers.get(pages.$package$.$className$Page) map {";\
     print "    answer =>";\
     print "      Row(";\
     ...
     print "            href               = controllers.$package$.routes.$className$Controller.onPageLoad(CheckMode).url,";\
