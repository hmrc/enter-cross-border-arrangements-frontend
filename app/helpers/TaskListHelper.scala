...
object TaskListHelper  {
  ...
  def taskListItemRestricted(linkContent: String, ariaLabel: String, itemType: String)(implicit messages: Messages): Html = {
    Html(s"<li class='app-task-list__$itemType'><a class='app-task-list__task-name' aria-describedby='$ariaLabel'> ${messages(linkContent)}</a>" +
  ...
  def taskListItemLinkedProvider(url: String, status: String, linkContent: String, id: String, ariaLabel: String, itemType: String)(implicit messages: Messages): Html = {
    Html(s"<li class='app-task-list__$itemType'><a class='app-task-list__task-name' href='$url' aria-describedby='$ariaLabel'> ${messages(linkContent)}</a>" +
      s"<strong class='govuk-tag app-task-list__task-completed' id='$id'>$status</strong> </li>")
  }

  def taskListItemNotLinkedProvider(status: String, linkContent: String, id: String, ariaLabel: String, itemType: String)(implicit messages: Messages): Html = {
    Html(s"<li class='app-task-list__$itemType'><a class='app-task-list__task-name' aria-describedby='$ariaLabel'> ${messages(linkContent)}</a>" +
      s"<strong class='govuk-tag app-task-list__task-completed' id='$id'>$status</strong> </li>")
  }

  def retrieveRowWithStatus(ua: UserAnswers, page: QuestionPage[JourneyStatus],
                            url: String, linkContent: String, id: String, ariaLabel: String, itemType: String, index: Int)(implicit messages: Messages): Html = {
    ua.get(page, index) match {
      case Some(Completed) => taskListItemLinkedProvider(url, Completed.toString, linkContent, s"$id-completed", ariaLabel, itemType)
      case Some(InProgress) => taskListItemLinkedProvider(url, InProgress.toString, linkContent, s"$id-inProgress", ariaLabel, itemType)
      case _ => taskListItemLinkedProvider(url, NotStarted.toString, linkContent, s"$id-notStarted", ariaLabel, itemType)
    }
  }
