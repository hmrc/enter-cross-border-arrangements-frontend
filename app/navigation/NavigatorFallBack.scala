...
class NavigatorFallBack @Inject()() {
  ...
  private val normalRoutes: Page => UserAnswers => Int => Request[AnyContent] => Option[Call] = {
    ...
    case SelectAnyTaxpayersThisEnterpriseIsAssociatedWithPage => _ => id => _ => Some(controllers.enterprises.routes.AssociatedEnterpriseTypeController.onPageLoad(id, NormalMode)) // TODO redirect
    case AssociatedEnterpriseTypePage => associatedEnterpriseTypeRoutes(NormalMode)
    case IsAssociatedEnterpriseAffectedPage => _ => id => _ =>
      Some(controllers.enterprises.routes.AssociatedEnterpriseCheckYourAnswersController.onPageLoad(id))
  ...
  private val checkRouteMap: Page => UserAnswers => Int => Request[AnyContent] => Option[Call] = {
    ...
    case SelectAnyTaxpayersThisEnterpriseIsAssociatedWithPage => _ => id => _ => Some(controllers.enterprises.routes.AssociatedEnterpriseTypeController.onPageLoad(id, CheckMode)) // TODO redirect
 ...
 def catRoutes(key: HallmarkCategories): (Int, Mode) => Call = key match {
  ...
  private def hallmarkDRoutes(mode: Mode)(ua: UserAnswers)(id: Int)(request: Request[AnyContent]): Option[Call] =
    ua.get(HallmarkDPage, id) flatMap  {
      case set: Set[HallmarkD] if set.contains(D1) => Some(controllers.hallmarks.routes.HallmarkD1Controller.onPageLoad(id, mode))
       case  _ => ua.get(HallmarkCategoriesPage, id).map {
         case set: Set[HallmarkCategories] if set.contains(CategoryE) =>
           controllers.hallmarks.routes.HallmarkEController.onPageLoad(id, mode)
         case _ => controllers.hallmarks.routes.CheckYourAnswersHallmarksController.onPageLoad(id)
       }
    }

  private def hallmarkD1Routes(mode: Mode)(ua: UserAnswers)(id: Int)(request: Request[AnyContent]): Option[Call] =
    ua.get(HallmarkD1Page, id) flatMap  {
      case set: Set[HallmarkD1] if set.contains(D1other) => Some(controllers.hallmarks.routes.HallmarkD1OtherController.onPageLoad(id, mode))
      case  _ => ua.get(HallmarkCategoriesPage, id).map {
        case set: Set[HallmarkCategories] if set.contains(CategoryE) =>
          controllers.hallmarks.routes.HallmarkEController.onPageLoad(id, mode)
        case _ => controllers.hallmarks.routes.CheckYourAnswersHallmarksController.onPageLoad(id)
  ...
  private def updateTaxpayerRoutes(mode: Mode)(ua: UserAnswers)(id: Int)(request: Request[AnyContent]): Option[Call] =
    ua.get(UpdateTaxpayerPage, id) map {
      case Now => controllers.taxpayer.routes.TaxpayerSelectTypeController.onPageLoad(id, mode)
  ...
  private def youHaveNotAddedAnyAssociatedEnterprisesPage(mode: Mode)(ua: UserAnswers)(id: Int)(request: Request[AnyContent]): Option[Call] =
    ua.get(YouHaveNotAddedAnyAssociatedEnterprisesPage, id) map {
      case YouHaveNotAddedAnyAssociatedEnterprises.YesAddNow  =>
        controllers.enterprises.routes.SelectAnyTaxpayersThisEnterpriseIsAssociatedWithController.onPageLoad(id, mode)
      case _ =>
        controllers.enterprises.routes.YouHaveNotAddedAnyAssociatedEnterprisesController.onPageLoad(id, mode)
    }

  private def associatedEnterpriseTypeRoutes(mode: Mode)(ua: UserAnswers)(id: Int)(request: Request[AnyContent]): Option[Call] = {
    ua.get(AssociatedEnterpriseTypePage, id) map {
