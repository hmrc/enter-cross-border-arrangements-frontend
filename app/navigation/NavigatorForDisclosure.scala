...
class NavigatorForDisclosure @Inject()() extends AbstractNavigator {
  ...
  override val routeMap:  Page => CheckRoute => Option[Any] => Int => Call = {
    ...
    case DisclosureMarketablePage => checkRoute => value => _ => routes.DisclosureCheckYourAnswersController.onPageLoad(checkRoute.mode)
    case DisclosureIdentifyArrangementPage => checkRoute => _ => _ => controllers.disclosure.routes.DisclosureCheckYourAnswersController.onPageLoad(checkRoute.mode)
  }
