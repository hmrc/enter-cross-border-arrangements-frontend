...
class NavigatorForReporter @Inject()() extends AbstractNavigator {
  ...
  override val routeMap:  Page => CheckRoute => Int => Option[Any] => Int => Call = {
    ...
    case ReporterOrganisationOrIndividualPage => checkRoute => id => value => _ => value match {
    ...
    case ReporterTinUKQuestionPage => checkRoute => id => value => index => value match {
    ...
    case ReporterTinNonUKQuestionPage => checkRoute => id => value => index => value match {
      case Some(true) => controllers.reporter.routes.ReporterNonUKTaxNumbersController.onPageLoad(id, checkRoute.mode, index)
      case _ => controllers.reporter.routes.ReporterOtherTaxResidentQuestionController.onPageLoad(id, checkRoute.mode, index + 1)
    }

    case ReporterUKTaxNumbersPage | ReporterNonUKTaxNumbersPage => checkRoute => id => _ => index =>
      controllers.reporter.routes.ReporterOtherTaxResidentQuestionController.onPageLoad(id, checkRoute.mode, index + 1)

    case ReporterOtherTaxResidentQuestionPage => checkRoute => id => value => index => value match {
    ...
    case RoleInArrangementPage => checkRoute => id => value => _ => value match {
    ...
    case IntermediaryWhyReportInUKPage => checkRoute => id => _ => _ =>
      controllers.reporter.intermediary.routes.IntermediaryRoleController.onPageLoad(id, checkRoute.mode)

    case IntermediaryRolePage => checkRoute => id => _ =>_ =>
      jumpOrCheckYourAnswers(id, controllers.reporter.intermediary.routes.IntermediaryExemptionInEUController.onPageLoad(id, checkRoute.mode), checkRoute)

    case IntermediaryExemptionInEUPage => checkRoute => id => value => _ => value match {
    ...
    case IntermediaryDoYouKnowExemptionsPage => checkRoute => id => value => _ => value match {
    ...
    case IntermediaryWhichCountriesExemptPage => checkRoute => id => _ => _ =>
      jumpOrCheckYourAnswers(id, controllers.reporter.routes.ReporterCheckYourAnswersController.onPageLoad(id), checkRoute)

    case ReporterTaxpayersStartDateForImplementingArrangementPage => _ => id => _ => _ =>
      controllers.reporter.routes.ReporterCheckYourAnswersController.onPageLoad(id)

    // Reporter - Taxpayer Journey Navigation

    case TaxpayerWhyReportInUKPage => checkRoute => id => value =>_ => value match {
      case Some(DoNotKnow) =>
    ...
    case TaxpayerWhyReportArrangementPage => checkRoute => id => _ =>_ =>
      jumpOrCheckYourAnswers(id, controllers.reporter.taxpayer.routes.ReporterTaxpayersMarketableArrangementGatewayController.onRouting(id, checkRoute.mode), checkRoute)
    ...
    case ReporterOrganisationNamePage =>
      checkRoute => id => _ => _ =>
        jumpOrCheckYourAnswers(id, controllers.reporter.organisation.routes.ReporterOrganisationIsAddressUkController.onPageLoad(id, checkRoute.mode), checkRoute)

    case ReporterOrganisationIsAddressUkPage => checkRoute => id => value => _ => value match {
      case Some(true) =>
    ...
    case ReporterOrganisationPostcodePage => checkRoute => id => _ => _ =>
      controllers.reporter.organisation.routes.ReporterOrganisationSelectAddressController.onPageLoad(id, checkRoute.mode)

    case ReporterOrganisationSelectAddressPage | ReporterOrganisationAddressPage => checkRoute => id => _ => _ =>
      jumpOrCheckYourAnswers(id, controllers.reporter.organisation.routes.ReporterOrganisationEmailAddressQuestionController.onPageLoad(id, checkRoute.mode), checkRoute)

    case ReporterOrganisationEmailAddressQuestionPage => checkRoute => id => value => _ => value match {
      case Some(true) =>
    ...
    case ReporterOrganisationEmailAddressPage => checkRoute => id => _ => index =>
      jumpOrCheckYourAnswers(id, controllers.reporter.routes.ReporterTaxResidentCountryController.onPageLoad(id, checkRoute.mode, 0), checkRoute)

    // Reporter - Individual Journey Navigation

    case ReporterIndividualNamePage => checkRoute => id => _ => _ =>
      jumpOrCheckYourAnswers(id, controllers.reporter.individual.routes.ReporterIndividualDateOfBirthController.onPageLoad(id, checkRoute.mode), checkRoute)

    case ReporterIndividualDateOfBirthPage => checkRoute => id => _ => _ =>
      jumpOrCheckYourAnswers(id, controllers.reporter.individual.routes.ReporterIndividualPlaceOfBirthController.onPageLoad(id, checkRoute.mode), checkRoute)

    case ReporterIndividualPlaceOfBirthPage => checkRoute => id => _ => _ =>
      jumpOrCheckYourAnswers(id, controllers.reporter.individual.routes.ReporterIsIndividualAddressUKController.onPageLoad(id, checkRoute.mode), checkRoute)

    case ReporterIsIndividualAddressUKPage => checkRoute => id => value => _ => value match {
    ...
    case ReporterIndividualPostcodePage => checkRoute => id => _ => _ =>
      controllers.reporter.individual.routes.ReporterIndividualSelectAddressController.onPageLoad(id, checkRoute.mode)

    case ReporterIndividualAddressPage | ReporterIndividualSelectAddressPage => checkRoute => id => _ => _ =>
      jumpOrCheckYourAnswers(id, controllers.reporter.individual.routes.ReporterIndividualEmailAddressQuestionController.onPageLoad(id, checkRoute.mode), checkRoute)

    case ReporterIndividualEmailAddressQuestionPage => checkRoute => id => value => _ => value match {
      case Some(true) =>
    ...
    case ReporterIndividualEmailAddressPage => checkRoute => id => _ => _ =>
      jumpOrCheckYourAnswers(id, controllers.reporter.routes.ReporterTaxResidentCountryController.onPageLoad(id, checkRoute.mode, 0), checkRoute)

    case ReporterCheckYourAnswersPage => _ => id => _ => _ =>
      controllers.routes.DisclosureDetailsController.onPageLoad(id)

    case _ => checkRoute => id => _ => _ => checkRoute.mode match {
