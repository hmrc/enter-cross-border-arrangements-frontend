  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      val json = Json.obj(
        "form"       -> preparedForm,
        "mode"       -> mode,
        "checkboxes" -> SelectAnyTaxpayersThisEnterpriseIsAssociatedWith.checkboxes(preparedForm)
      )
      ...
      form.bindFromRequest().fold(
        formWithErrors => {
          ...
          val json = Json.obj(
            "form"       -> formWithErrors,
            "mode"       -> mode,
            "checkboxes" -> SelectAnyTaxpayersThisEnterpriseIsAssociatedWith.checkboxes(formWithErrors)
          )
