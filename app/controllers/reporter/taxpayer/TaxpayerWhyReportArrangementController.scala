...
class TaxpayerWhyReportArrangementController @Inject()(
    override val messagesApi: MessagesApi,
    sessionRepository: SessionRepository,
    identify: IdentifierAction,
    getData: DataRetrievalAction,
    requireData: DataRequiredAction,
    formProvider: TaxpayerWhyReportArrangementFormProvider,
  ...
  def redirect(mode:Mode, value: Option[TaxpayerWhyReportArrangement], index: Int = 0, alternative: Boolean = false): Call =
    NavigatorForReporter.nextPage(TaxpayerWhyReportArrangementPage, mode, value, index, alternative)
    ...
    implicit request =>
      ...
      form.bindFromRequest().fold(
        formWithErrors => {
          ...
          renderer.render("reporter/taxpayer/taxpayerWhyReportArrangement.njk", json).map(BadRequest(_))
        },
        value =>
          for {
