...
class ReporterIndividualPostcodeController @Inject()(
  ...
  private def manualAddressURL(mode: Mode): String = controllers.routes.IndexController.onPageLoad().url //Todo update to correct value
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      val json = Json.obj(
        "form" -> preparedForm,
        "mode" -> mode,
        "manualAddressURL" -> manualAddressURL(mode) //TODO When page is ready
      )
      ...
      form.bindFromRequest().fold(
        formWithErrors => {
          val json = Json.obj(
            "form" -> formWithErrors,
            "mode" -> mode,
            "manualAddressURL" -> manualAddressURL(mode) //TODO
          )
