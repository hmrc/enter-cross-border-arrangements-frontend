...
final case class UserAnswers(
  ...
  def set[A, M](page: DetailsPage[A, M], index: Int)(implicit writes: Writes[A], model: M): Try[UserAnswers] =
    page.getFromModel(model).fold[Try[UserAnswers]](Success(this)) {
      value =>
        set(UnsubmittedIndex.fromQuestionPage(page, index)(this), value)
    }
  ...
  def set(loopPage: LoopDetailsPage, index: Int, position: Int)(f: LoopDetails => LoopDetails): Try[UserAnswers] = {
    val loopDetails = get(loopPage, index).fold(IndexedSeq[LoopDetails](f(LoopDetails()))) {
      list =>
        list
          .lift(position)
          .map(f)
          .fold(list) {
            updatedLoop => list.updated(position, updatedLoop)
          }
    }
  ...
  def set(loopPage: LoopDetailsPage, index: Int)(implicit model: WithTaxResidency): Try[UserAnswers] =
    Option(model.taxResidencies.map(LoopDetails.apply)).fold[Try[UserAnswers]](Success(this)) {
      value =>
  ...
  def removeAll(pageList: Seq[_ <: QuestionPage[Any]]): Try[UserAnswers] =
    pageList.foldLeft(removeBase(pageList.head)) {
      case (result, step) => result.flatMap(_.removeBase(step))
    }
      ...
      .flatMap {
        nonEmptyItemId =>
          this
            ...
            .map(_.asInstanceOf[WithRestore].restore(this, id).getOrElse(throw new SomeInformationIsMissingException(id, Some("Unable to restore from loop"))))
      }
      .getOrElse(this)
