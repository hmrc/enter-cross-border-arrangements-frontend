...
class WhichExpectedInvolvedCountriesArrangementFormProvider @Inject() extends Mappings {
  ...
  def apply(): Form[Set[WhichExpectedInvolvedCountriesArrangement]] =
    Form(
      "value" -> set(enumerable[WhichExpectedInvolvedCountriesArrangement]("whichExpectedInvolvedCountriesArrangement.error.required")).verifying(nonEmptySet("whichExpectedInvolvedCountriesArrangement.error.required"))
    )
}
