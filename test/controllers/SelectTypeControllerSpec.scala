...
class SelectTypeControllerSpec extends SpecBase with MockitoSugar with NunjucksSupport with JsonMatchers {
  ...
  "SelectType Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val filledForm = form.bind(Map("value" -> SelectType.values.head.toString))
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, selectTypeRoute)
          .withFormUrlEncodedBody(("value", SelectType.values.head.toString))
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
      val request = FakeRequest(POST, selectTypeRoute).withFormUrlEncodedBody(("value", "invalid value"))
