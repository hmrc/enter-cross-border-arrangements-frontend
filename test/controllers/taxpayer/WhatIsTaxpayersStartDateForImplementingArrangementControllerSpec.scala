...
class WhatIsTaxpayersStartDateForImplementingArrangementControllerSpec extends SpecBase with MockitoSugar with NunjucksSupport with JsonMatchers {
  ...
  "WhatIsTaxpayersStartDateForImplementingArrangement Controller" - {
    ...
    "must redirect to Select Type if TaxpayerSelectTypePage is missing" in {
      val application = applicationBuilder(Some(emptyUserAnswers)).build()

      val result = route(application, getRequest).value

      status(result) mustEqual SEE_OTHER
      redirectLocation(result).value mustEqual controllers.taxpayer.routes.TaxpayerSelectTypeController.onPageLoad(NormalMode).url
