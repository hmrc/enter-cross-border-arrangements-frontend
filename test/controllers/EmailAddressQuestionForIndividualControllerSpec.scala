...
import pages.EmailAddressQuestionForIndividualPage
...
class EmailAddressQuestionForIndividualControllerSpec extends SpecBase with MockitoSugar with NunjucksSupport with JsonMatchers {
  ...
  "EmailAddressQuestionForIndividual Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, emailAddressQuestionForIndividualRoute)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
      val request = FakeRequest(POST, emailAddressQuestionForIndividualRoute).withFormUrlEncodedBody(("confirm", ""))
    ...
    "must redirect to the Check your answers page when user change their answer to `No`" in {
      ...
      val mockSessionRepository = mock[SessionRepository]
      val individualDateOfBirthRouteCheck = routes.EmailAddressQuestionForIndividualController.onPageLoad(CheckMode)
      val userAnswers = UserAnswers(userAnswersId).set(EmailAddressQuestionForIndividualPage, false).success.value
