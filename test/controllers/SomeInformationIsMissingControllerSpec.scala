...
class SomeInformationIsMissingControllerSpec extends SpecBase with ControllerMockFixtures with NunjucksSupport with JsonMatchers {
  ...
  "SomeInformationIsMissing Controller" - {
    ...
    "return OK and the correct view" in {
      ...
      val request        = FakeRequest(GET, routes.SomeInformationIsMissingController.fromOther(0).url)
      val templateCaptor = ArgumentCaptor.forClass(classOf[String])
      val jsonCaptor     = ArgumentCaptor.forClass(classOf[JsObject])

      val expectedJson = Json.obj(
        "link" -> "/disclose-cross-border-arrangements/manual/your-disclosure-details/0"
      )
